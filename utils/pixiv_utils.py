    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status != 200:
                    yield event.plain_result(f"动图下载失败，状态码: {response.status}")
                    return
                
                zip_data = await response.read()
        
        # 生成安全的文件名
        safe_title = "".join(c for c in illust.title if c.isalnum() or c in (" ", "_", "-")).rstrip()
        if not safe_title:
            safe_title = "ugoira"
        ...
        if gif_result:
            ...
            try:
                ...
                logger.error(f"Pixiv 插件：发送GIF文件时发生错误 - {e}")
                yield event.plain_result(f"发送动图GIF时发生错误: {str(e)}")
        else:
            # GIF转换失败，只发送错误信息，不再发送ZIP文件
            yield event.plain_result(f"动图GIF转换失败，无法发送作品 {illust.id}。请确保系统已安装ffmpeg。")
...
async def _convert_ugoira_to_gif(zip_data, metadata, safe_title, illust_id):
    ...
    try:
        ...
        for i, frame in enumerate(metadata.frames):
            ...
            if frame_file:
                duration = frame.delay if hasattr(frame, 'delay') else 100  # 默认100ms
        ...
        async with aiohttp.ClientSession() as session:
            for img in batch_imgs:
                ...
                if hasattr(img, 'type') and img.type == 'ugoira':
                    ...
                    try:
                        # 获取动图元数据
                        ugoira_metadata = await asyncio.to_thread(client.ugoira_metadata, img.id)
                        if ugoira_metadata and hasattr(ugoira_metadata, 'ugoira_metadata'):
                            metadata = ugoira_metadata.ugoira_metadata
                            if hasattr(metadata, 'zip_urls') and metadata.zip_urls.medium:
                                zip_url = metadata.zip_urls.medium
                                ...
                                async with session.get(
                                    ...
                                    if response.status == 200:
                                        ...
                                        # 生成安全的文件名
                                        safe_title = "".join(c for c in img.title if c.isalnum() or c in (" ", "_", "-")).rstrip()
                                        if not safe_title:
                                            safe_title = "ugoira"
                                        
                                        # 尝试转换为GIF
                                        gif_result = await _convert_ugoira_to_gif(zip_data, metadata, safe_title, img.id)
                                        ...
                                        if gif_result:
                                            ...
                                            try:
                                                # 构建GIF信息消息
                                                ugoira_info = "🎬 动图作品（已转换为GIF）\n"
                                                ugoira_info += f"标题: {img.title}\n"
                                                ugoira_info += f"作者: {img.user.name}\n"
                                                ugoira_info += f"帧数: {len(metadata.frames)}\n"
                                                ugoira_info += f"GIF大小: {gif_info.get('size', 0) / 1024 / 1024:.2f} MB\n"
                                                ...
                                                ugoira_info += f"作品链接: https://www.pixiv.net/artworks/{img.id}\n\n"
                                                ugoira_info += "💡 动图已转换为GIF格式，可以直接播放。"
                                                ...
                                                logger.error(f"Pixiv 插件：转发消息中发送GIF文件时发生错误 - {e}")
                                                node_content = [Plain(f"发送动图GIF时发生错误: {str(e)}")]
                                        else:
                                            # GIF转换失败
                                            node_content = [Plain(f"动图GIF转换失败，无法发送作品 {img.id}。请确保系统已安装ffmpeg。")]
                                    else:
                                        node_content = [Plain(f"动图下载失败，状态码: {response.status}")]
                        else:
                            node_content = [Plain("无法获取动图元数据")]
                            
                    except Exception as e:
                        logger.error(f"Pixiv 插件：处理转发消息中的动图时发生错误 - {e}")
                        node_content = [Plain(f"处理动图时发生错误: {str(e)}")]
                else:
                    # 处理普通图片
                    detail_message = build_detail_message_func(img)
                    ...
                    node_content = []
                    filename = _temp_dir / f"pixiv_{uuid.uuid4().hex}.jpg"
                    if image_url:
                        try:
                            async with session.get(image_url, headers=headers, proxy=_config.proxy or None) as resp:
                                if resp.status == 200:
