...
def filter_illusts_with_reason(illusts, r18_mode, ai_filter_mode, display_tag_str=None, first_tag=None, all_illusts_from_first_tag=None, return_count=1, logger=None, show_filter_result=True):
    """
    ...
    if filtered_count < initial_count:
        filter_reasons = []
        if r18_mode == "过滤 R18" or r18_mode == "仅 R18": filter_reasons.append("R18")
        if ai_filter_mode == "过滤 AI 作品" or ai_filter_mode == "仅 AI 作品": filter_reasons.append("AI")
        if filter_reasons:
            filter_msgs.append(f"部分作品因 R18/AI 设置被过滤 (找到 {initial_count} 个符合所有标签的作品，最终剩 {filtered_count} 个可发送)。")
    elif initial_count > 0:
        filter_msgs.append(f"筛选完成，共找到 {initial_count} 个符合所有标签「{display_tag_str or ''}」的作品。正在发送最多 {return_count} 张...")
    # 结果为空时详细原因
    if not filtered_list:
        ...
        if no_result_reason and initial_count > 0:
            filter_msgs.append(f"所有找到的作品均为 {' 或 '.join(no_result_reason)}，根据当前设置已被过滤。")
        elif initial_count == 0 and all_illusts_from_first_tag is not None and len(all_illusts_from_first_tag) > 0:
            filter_msgs.append(f"找到了与「{first_tag}」相关的作品，但没有作品同时包含所有标签「{display_tag_str}」。")
        elif initial_count == 0 and all_illusts_from_first_tag is not None and len(all_illusts_from_first_tag) == 0:
            filter_msgs.append(f"未找到任何与标签「{first_tag}」相关的作品。")
        else:
            if logger: logger.warning("AND 深度搜索后没有符合条件的插画可供发送，但过滤原因不明确。")
